...
const App: React.FC = () => {
  ...
  const [lastActiveRoom, setLastActiveRoom] = useState<RoomType>(RoomType.AUDIO); // Default to Audio/Center
  const [isTransitioning, setIsTransitioning] = useState<boolean>(false);

  const handleEnterRoom = (room: RoomType) => {
    if (isTransitioning) return;
    setLastActiveRoom(room); // Remember this room
